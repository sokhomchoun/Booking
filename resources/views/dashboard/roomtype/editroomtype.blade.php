...
<body>
    ...
    {{-- end menu  --}}
     ...
     <main id="main" class="main">
        <h3 id="add_cate" class="mb-3"><span class="icon_save"><i class="bi bi-house-add"></i></span>RoomType</h3>
          <form action="{{ url('/updatetype/'.$item->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="category_header">
              <div class="mb-3">
                <label for="" class="form-label">Type No</label>
                <input type="text" class="form-control" id="clearId" name="room_no" value="{{$item->type_no}}">
              </div>
              <div class="mb-3">
                <label for="" class="form-label">Bed</label>
                <input type="text" class="form-control" id="clearName" name="bed_number" value="{{$item->bed_number}}">
              </div>
              <div class="mb-3">
                <label for="" class="form-label">Price Per Night</label>
                <input type="text" class="form-control" id="clearName" name="price_per_night" value="{{$item->price_per_night}}">
              </div>
              <div class="mb-3">
                <label for="" class="form-label">Capacity</label>
                <input type="text" class="form-control" id="clearName" name="capacity" value="{{$item->capacity}}">
              </div>

              <div class="mb-3">
                <label for="" class="form-label">Description</label>
                <input type="text" class="form-control" id="clearName" name="description" value="{{$item->description}}">
                {{-- <textarea class="form-control" id="" rows="3" name="description" value="{{$item->description}}"></textarea> --}}
              </div>
